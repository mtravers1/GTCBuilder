        ...
        <div className="mt-24">
            ...
            <Container size="xl" >
                <div className="relative">
                    {/* Main images gallery */}
                    <div>
                        <MainImagesGalery data={property.images} />
                    </div>

                    {/* Bookmark */}
                    <div className='absolute top-0 right-0 p-1'>
                        <button onClick={() => bookmark()} >
                            <Image src={isBookmark ? "/icons/bookmarkFill.svg" : "/icons/bookmark.svg"}
                                width={30}
                                height={30}
                                alt="icon"
                            />
                ...
                <div>
                    <div className="flex justify-between space-x-2 my-2">
                        {/* left */}
                        {/* Title */}
                        <h1 className="font-bold pc:text-2xl tablet:text-xl text-base">{property.title}</h1>
                    ...
                    {/* Location */}
                    <div className="text-black/80 italic pc:text-base text-sm mb-2">Gampaha,Sri Lanka</div>
                    {/* Badges */}
                    <div className="flex flex-wrap tablet:space-y-0 tablet:flex-row space-y-2 tablet:space-x-2 flex-col w-full overflow-x-hidden whitespace-nowrap ">
                        <Badge size="lg" className="bg-primary" variant="filled">Apartment</Badge>
                        <Badge size="lg" className="bg-primary" variant="filled">Rent</Badge>
                        <Badge size="lg" className="bg-primary" variant="filled">Negotiable</Badge>
                    </div>
                </div>
...
export async function getServerSideProps({ query }) {
    ...
    const property = PropertiesData.find(e => e.slug === query.id)
